...
def quantile_bucket_returns(
    ...
    *,
    ...
    joined = (
        ...
        .with_columns(
            (
                ((pl.col("rank_in_date") - 1) * buckets / pl.col("date_count")).floor()
                + 1
            )
            .clip(1, buckets)
            .cast(pl.Int64)
...
def long_short_by_date(
    bucket_returns: pl.DataFrame, *, top_bucket: int = 20, bottom_bucket: int = 1
) -> pl.DataFrame:
    """Return top-minus-bottom bucket spread by date."""
    wide = bucket_returns.filter(
        pl.col("bucket").is_in([bottom_bucket, top_bucket])
    ).pivot(values="mean_forward_return", index="date", on="bucket")
    top = str(top_bucket)
    bottom = str(bottom_bucket)
    return wide.with_columns(
        (pl.col(top) - pl.col(bottom)).alias("long_short_return")
    ).select("date", "long_short_return")
